...
<script setup lang="ts">
  ...
  store.load()
 	...
 	async function load() {
      axios.get('https://api.cosmicjs.com/v3/buckets/comomayacom-production/objects?pretty=true&query=%7B%22type%22:%22portfolios%22%7D&read_key=Yz8ifYSRHxv4SzRygKNMbdGZnUaTUAUZBbseBGOILB3eWpiwh1&depth=1&props=slug,title,metadata,content', { withCredentials: false })
  ...
  <main class="leading-relaxed min-h-screen py-32 bg-beige-lighter z-0 relative px-9 md:px-20 xl:px-36">
    <transition-group tag="section" v-if="thisPage && Object.keys(thisPage.content).length" class="flex flex-col md:justify-start md:flex-row justify-center first">
        ...
        <div key="info" class="info md:w-1/3">
          <h1 class="text-black text-6xl font-extrabold leading-[0.85em] md:mt-0 mt-3 mb-5 lowercase">{{thisPage.content.title}}</h1>
