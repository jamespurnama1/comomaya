        ...
        <li v-for="(portfolio, i) in filteredFunc()" :key="response.list.toString()" @mouseover="hov[i] = true"
          ...
          <a :href="`/work/${portfolio.slug}`" :aria-label="`Go to ${portfolio.title}`">
            ...
            <div class="flex items-center mt-2 select">
              ...
              <span>
                <h2 class="whitespace-nowrap origin-left transition-all"
                  :class="[hov[i] ? 'text-active font-extrabold text-5xl md:text-4xl lg:text-5xl lowercase tracking-[-0.035em]' : 'text-beige-lighter tracking-widest text-lg uppercase font-semibold']">
                  {{ portfolio.title }}</h2>
              </span>
            </div>
          </a>
